...
async def init_db_test() -> None:
    ...
    logger.info(await conn_2.fetch('''show timezone;'''))
    try:
    ...
    try:
        ...
        for _ in range(count):
            ...
            company_in_obj = await crud.company.create(db, obj_in=company_in)

            for u in range(2):
                user_in_client = schemas.UserCreate(
                    email=''.join(random.choice(string.ascii_letters) for _ in range(10)) + "@gmail.com",
                ...
                for t in range(2):
                    ...
                    task_in = schemas.TaskCreate(
                        ...
                        executor_id=random.choice([user_obj_ranker.id, user_obj_ranker.id, user_obj_manager.id]),
